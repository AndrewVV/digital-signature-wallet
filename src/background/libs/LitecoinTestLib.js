...
const LTCDECIMALS = 8;
class LitecoinTestLibClass{
    constructor(wallet){
        console.log('Litecoin Testnet Active');
    ...
    getBalance(raw=true){
        return new Promise(async(resolve,reject)=>{
            try{
                ...
                let url = `https://chain.so/api/v2/get_address_balance/LTCTEST/${address}`
    ...
    sendTransaction(to,amount,fee){
        return new Promise(async(resolve,reject)=>{
            try{
                ...
                let url = `https://chain.so/api/v2/send_tx/LTCTEST`
    ...
    listUnspent(address){
        return new Promise(async(resolve,reject)=>{
            try{
   	            this.validator.validateBtcAddress(address);
   	            let url = `https://chain.so/api/v2/get_tx_unspent/LTCTEST/${address}`
...
module.exports = LitecoinTestLibClass;
