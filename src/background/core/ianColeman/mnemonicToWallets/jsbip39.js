...
class Mnemonic {
    ...
    init() {
        if (WORDLISTS.length != RADIX) {
            let err = 'Wordlist should contain ' + RADIX + ' words, but it contains ' + WORDLISTS.length + ' words.';
            throw err;
        }
    }
    ...
    toMnemonic(byteArray) {
        ...
        var data = this.byteArrayToWordArray(byteArray);
        ...
        // d = c[:len(data) * 8 / 32]
        var a = this.byteArrayToBinaryString(byteArray);
    ...
    toSeed(mnemonic, passphrase) {
        ...
        var result = sjcl.misc.pbkdf2(mnemonicBits, passphraseBits, RADIX, 512, hmacSHA512);
    ...
    hexStringToBinaryString(hexString) {
        var binaryString = "";
    ...
    binaryStringToWordArray(binary) {
